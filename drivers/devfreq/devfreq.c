...
static ssize_t active_freq_store(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t count)
{
	struct devfreq *df = to_devfreq(dev);
	unsigned long value;
...
show_one(min_freq);
show_one(max_freq);
show_one(active_freq);
static DEVICE_ATTR_RW(min_freq);
static DEVICE_ATTR_RW(max_freq);
static DEVICE_ATTR_RW(active_freq);
